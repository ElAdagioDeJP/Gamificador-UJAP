...
// Función principal
function main() {
  log(`${colors.bright}${colors.blue}🔄 Sincronizando configuración de IP...${colors.reset}\n`);
  ...
  log(`\n${colors.bright}${colors.green}✅ Configuración sincronizada exitosamente!${colors.reset}`);
  ...
  log(`${colors.yellow}  npm run dev${colors.reset} - Para iniciar el backend`);
...
if (args.includes('--help') || args.includes('-h')) {
  log(`${colors.bright}Uso: node scripts/sync-ip.js [opciones]${colors.reset}\n`);
...
if (args.includes('--ip')) {
  ...
  if (customIP) {
    log(`${colors.bright}${colors.blue}🔄 Sincronizando configuración con IP personalizada: ${customIP}${colors.reset}\n`);
    updateBackendEnv(customIP);
    updateFrontendEnv(customIP);
    log(`\n${colors.bright}${colors.green}✅ Configuración sincronizada con IP: ${customIP}${colors.reset}`);
  } else {
